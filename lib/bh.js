        ...
        for (var blockName in declByBlock) {
            ...
            for (var elemName in declsByElem) {
                ...
                for (j = 0, l = decls.length; j < l; j++) {
                    ...
                    if (decl.elemMod) {
                        conds.push(
                            'json.mods["' + strEscape(decl.elemMod) + '"] === ' +
            ...
            if (Array.isArray(json)) {
                ...
                if (json.block) {
                    ...
                    if (!json._stop) {
                        ...
                        ctx.node = node;
                        ctx.ctx = json;
                        [json].concat(json.mix)
                            .forEach(function(item, i) {
                                console.log('i', i)
                                if (item) {
                                    if (i > 0) {
                                        json.block = item.block;
                                        json.elem = item.elem;
                                        json.mods = item.mods || {};
                                        json.blockMods = item.blockMods || {};
                                        json.js = item.js;
                                        console.log('json after', json)
                                    }
                                    subRes = compiledMatcher(ctx, item);
                                        console.log('json after', json)
                                    if (i === 0) {
                                        base.block = json.block;
                                        base.elem = json.elem;
                                        base.mods = json.mods;
                                        base.blockMods = json.blockMods;
                                        base.js = json.js;
                                        console.log('base', base);
                                    } else {
                                        item.block = json.block;
                                        item.elem = json.elem;
                                        item.mods = json.mods || {};
                                        item.blockMods = json.blockMods || {};
                                        item.js = json.js;
                                    }
                                }
                                //console.log('i', i)
                        ...
                        json.block = base.block;
                        json.elem = base.elem;
                        json.mods = base.mods || {};
                        json.blockMods = base.blockMods || {};
                        json.js = base.js;
                    }
                }
                if (!stopProcess) {
