...
BH.prototype = {
     ...
     *        {String} options[jsClsName] Название класса, который который примешивается к блокам с `js`. По умолчанию, `i-bem`.     * 
    ...
    setOptions: function(options) {
        ...
        if (options.jsClsName) {
            this._optJsClsName = options.jsClsName;
        }
    ...
    toHtml: function(json) {
        ...
        if (typeof json !== 'object') {
            ...
            if (isBEM) {
                ...
                if (jsParams) {
                    if (addJSInitClass) cls += ' ' + this._optJsClsName;
