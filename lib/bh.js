    ...
    buildMatcher: function () {
        ...
        for (i = allMatchers.length - 1; i >= 0; i--) {
            ...
            if (currentBlock !== decl.block) {
                ...
                if (currentBlock) {
                    res.push('if (json.block === "' + currentBlock + '") {');
                }
            }
            var conds = [];
            decl.elem && conds.push('json.elem === "' + decl.elem + '"');
            decl.block && !decl.elem && conds.push('!json.elem');
            decl.mod && conds.push('json.mods && json.mods.' + decl.mod + ' === "' + decl.modVal + '"');
            if (decl.block && decl.blockMod) {
                conds.push('json.blockMods.' + decl.blockMod + ' === "' + decl.blockModVal + '"');
            }
            jsCond && conds.push(jsCond);
            conds.push('!json.' + mi[1].__id);
            res.push('if (' + conds.join(' && ') + ') {');
            res.push('json.' + mi[1].__id + ' = true;');
            res.push('subRes = _m' + i + '(ctx, json);');
