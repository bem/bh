function BH() {
    ...
    this.utils = {
        ...
        isLast: function () {
            var node = this.node;
            return node.index === 'content' ? true : node.index === node.arr.length - 1;
        },
...
BH.prototype = {
    ...
    buildMatcher: function () {
        ...
        for (i = allMatchers.length - 1; i >= 0; i--) {
            ...
            decl.mod && conds.push('ctx.mods.' + decl.mod + ' === "' + decl.modVal + '"');
    ...
    toHtml: function (json) {
        ...
        if (Array.isArray(json)) {
            ...
            if (jattr = json.attrs) {
                if (this._dirtyEnv) {
                    for (i in jattr) {
                        jattr.hasOwnProperty(i) && (attrs = attrs || []).push(i + '="' + this.escapeAttr(jattr[i]) + '"');
                    }
                } else {
                    for (i in jattr) {
                        (attrs = attrs || []).push(i + '="' + this.escapeAttr(jattr[i]) + '"');
                    }
                }
            }
            ...
            if (jsParams) {
                (attrs = attrs || []).push((json.jsAttr || 'onclick') + '="return ' +
            ...
            res = '<' + tag + (cls ? ' class="' + cls + '"' : '') + (attrs ? ' ' + attrs.join(' ') : '');
    ...
    toBEMCssClasses: function (json, blockName) {
        ...
        if (mods = json.mods) {
            if (this._dirtyEnv) {
                for (i in mods) {
                    mods.hasOwnProperty(i) && res.push(base + '_' + i + '_' + mods[i]);
                }
            } else {
                for (i in mods) {
        ...
        if ((mix = json.mix) && (l = mix.length)) {
            for (i = 0; i < l; i++) {
                res.push(this.toBEMCssClasses(mix[i], blockName))
            }
        }
        return res.join(' ');
    }
