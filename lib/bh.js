...
BH.prototype = {
    ...
    processBemjson: function (bemjson, blockName, ignoreContent) {
        ...
        /**
         * Обертка над текущим узлом JSON.
         * 
         * @name Ctx
         */
        function Ctx() {
...
var toHtml = function (json) {
    ...
    if (typeof json !== 'object') {
        ...
        if (jattr = json.attrs) {
            if (dirtyEnv) {
                for (i in jattr) {
                    jattr.hasOwnProperty(i) && (jattr[i] !== null) && (attrs += ' ' + i + '="' + escapeAttr(jattr[i]) + '"');
                }
            } else {
                for (i in jattr) {
                    (jattr[i] !== null) && (attrs += ' ' + i + '="' + escapeAttr(jattr[i]) + '"');
                }
            }
        }
...
var toBEMCssClasses = function (json, blockName) {
    ...
    if (mods = json.mods) {
        if (dirtyEnv) {
            for (i in mods) {
                mods.hasOwnProperty(i) && mods[i] && (res += ' ' + base + '_' + i + '_' + mods[i]);
            }
        } else {
            for (i in mods) {
                mods[i] && (res += ' ' + base + '_' + i + '_' + mods[i]);
            }
        }
    }
