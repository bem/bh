    ...
    this.utils = {
        ...
        applyCtx: function (changes) {
            ...
            if (changes) {
                prevValues = {};
                for (key in changes) {
                    if (this.bh._dirtyEnv && !changes.hasOwnProperty(key)) continue;
                    prevValues[key] = prevCtx[key];
                    prevCtx[key] = changes[key];
                }
            }
            var res = this.bh.processBemjson(this.ctx);
            if (res !== prevCtx) {
                this.newCtx = res;
            }
            if (changes) {
...
BH.prototype = {
     ...
     */
    apply: function (bemjson) {
        return toHtml(this.processBemjson(bemjson));
    },
        ...
        for (var blockName in declByBlock) {
            ...
            for (var elemName in declsByElem) {
                ...
                for (j = 0, l = decls.length; j < l; j++) {
                    ...
                    decl.mod && conds.push('json.mods && json.mods["' + escapeStr(decl.mod) + '"] === "' + escapeStr(decl.modVal) + '"');
                    if (decl.blockMod) {
                        conds.push('json.blockMods && json.blockMods["' + escapeStr(decl.blockMod) + '"] === "' + escapeStr(decl.blockModVal) + '"');
                    }
...
var toHtml = function (json) {
    ...
    if (typeof json !== 'object') {
        ...
        if (jattr = json.attrs) {
            if (dirtyEnv) {
                for (i in jattr) {
                    jattr.hasOwnProperty(i) && (attrs += ' ' + i + '="' + escapeAttr(jattr[i]) + '"');
                }
            } else {
                for (i in jattr) {
